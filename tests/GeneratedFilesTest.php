...
final class GeneratedFilesTest extends TestCase
{
    ...
    {
        $this->assertSame('foo', \Safe\sprintf('foo'));
        $this->assertSame('foobar', \Safe\sprintf('foo%s', 'bar'));
        $this->assertSame('foobarbaz', \Safe\sprintf('foo%s%s', 'bar', 'baz'));

        set_error_handler(function (): void {
        });
        try {
            $this->expectException(\ArgumentCountError::class);
            \Safe\sprintf('foo%s%s', 'bar');
        } finally {
